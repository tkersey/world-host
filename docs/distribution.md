...
The same lane also includes a cold-restore proof: the boot worker is disposed, a fresh worker/controller reads the committed TurnClosure from `MemoryStore`, and the restore TurnInput binds the exact parent closure fingerprint, state fingerprint, prior receipt fingerprint, closure bytes, and next sequence number.
