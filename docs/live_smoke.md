...
It is skipped unless `WORLD_HOST_LIVE_SMOKE=1` is set. It requires an explicit config path, secret provider, and allowlist. It defaults to dry-run unless `--live` is supplied. Destructive effects require `--allow-destructive`.
