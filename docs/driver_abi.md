...
A receiver capability pack does not provide an ABI module. With `adapter.kind = "receiver"`, world-host resolves the pack's `driverId` through its closed receiver registry and instantiates the registered host driver with the pack's `packFingerprint`. The driver manifest must then match the pack's driver ID, supported actuator references, descriptor fingerprints, actuation classes, response statuses, recovery class, byte limits, authority labels, and pack fingerprint exactly. Unknown driver IDs fail closed. `adapter.kind = "sidecar"` remains a separate out-of-process implementation of this contract; its artifacts are not imported into world-host.
