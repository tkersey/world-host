...
export function assertCapabilityPolicyAllows({ manifest, hostRequest = null, policy: inputPolicy = {}, mode = 'live', action = null }) {
  ...
  if (action?.destructive === true && policy.requireApprovalForDestructiveEffects) fail('ERR_CAPABILITY_APPROVAL_REQUIRED');
  if (isNetwork(manifest, hostRequest) && policy.requireApprovalForNetworkEffects) fail('ERR_CAPABILITY_APPROVAL_REQUIRED');
  if (manifest?.recoveryClass === EffectRecoveryClass.bestEffort && policy.requireApprovalForBestEffort) fail('ERR_CAPABILITY_APPROVAL_REQUIRED');
  return true;
}
