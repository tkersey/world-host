...
// src/drivers/fixture_agent_model_driver.mjs
var DEFAULT_ACTUATOR_REF = "fixture:agent-model";
...
class FixtureAgentModelDriver {
  ...
  async resolve(context, hostRequest) {
    ...
    if (key && this.responsesByKey.has(key)) {
      return { resolutionInputBytes: resolutionInput2(hostRequest, this.responsesByKey.get(key)) };
    }
    this.calls += 1;
    const prompt = parseDecisionPrompt(hostRequest.requestBytes);
    const response = actionForPrompt(this.scenario, prompt);
    const bytes2 = agentActionValueImage(response);
    if (key)
      this.responsesByKey.set(key, new Uint8Array(bytes2));
    return { resolutionInputBytes: resolutionInput2(hostRequest, bytes2) };
  }
  async recover(context, effectRecord) {
    if (!effectRecord.resolutionInputRef)
      return await this.resolve(context, effectRecord);
...
function parseDecisionPrompt(bytes2) {
  if (!(bytes2 instanceof Uint8Array))
    fail("ERR_AGENT_DECISION_PROMPT_BYTES_REQUIRED");
  try {
    const prompt = JSON.parse(new TextDecoder().decode(bytes2));
    if (prompt?.schema !== "boundary.Agent.DecisionPrompt.v0")
      fail("ERR_AGENT_DECISION_PROMPT_SCHEMA");
    if (typeof prompt.observation !== "string")
      fail("ERR_AGENT_DECISION_OBSERVATION_REQUIRED");
    return prompt;
  } catch (error) {
    if (error?.code)
      throw error;
...
function resolutionInput2(hostRequest, responseValueImageBytes, status = 0) {
  ...
  #resolution(hostRequest, payload, status, transactionRef) {
    ...
    return {
      resolutionInputBytes: encodeResolutionInputBytes({
        targetHostRequestFingerprint: resolutionTarget2(hostRequest),
        status,
        responseValueImageBytes,
...
function resolutionTarget2(hostRequest = {}) {
  const value = hostRequest.hostRequestFingerprint;
  if (typeof value === "bigint" || typeof value === "number")
    return BigInt(value);
...
// src/drivers/model_capability_driver.mjs
var DEFAULT_ALLOWED_TOOL_IDS = Object.freeze(["actuate", "read_file", "write_file"]);
var textDecoder = new TextDecoder;

class FixtureAgentModelCapabilityDriver extends FixtureAgentModelDriver {
  preflight(context, hostRequest) {
    const structural = defaultCapabilityPreflight(this.manifest(), hostRequest);
    const blockers = [...structural.blockers];
    if (!blockers.length) {
    ...
    if (blockers.length)
      return new CapabilityPreflightReport({ accepted: false, blockers });
    return structural;
  }
  dryRun(context, hostRequest) {
    const prompt = parseDecisionPrompt(hostRequest.requestBytes);
...
export {
  FixtureAgentModelCapabilityDriver as CapabilityDriver
};
